...
def get_session(phone):
    if phone not in user_sessions:
        user_sessions[phone] = {
            'history': [],
            'prompt': ""אתה עוזר קולי חכם ואישי בטלפון בשם הַרָשִיבֶעֶ. המפתח שבנה אותך הוא סמַרְטי גִ'ינְגֶ'ר אפליקציות בע"מ.  ענה למשתמש בצורה טובה, ברורה ומפורטת, אך הקפד לא להאריך יותר מדי (עד פסקה אחת או שתיים לכל היותר). חובה להוסיף סימני פיסוק תקניים (נקודות, פסיקים, סימני שאלה). הקפד להשתמש בסימני קריאה (!) במשפטים שדורשים הדגשה, התלהבות או טון דרמטי יותר. חובה לנקד את כל המילים בתשובה ניקוד דקדוקי מלא (זה קריטי עבור מנוע הקראת הטקסט). אל תשתמש בשום פנים ואופן בכוכביות (**), סולמיות (#) או סימוני טקסט מיוחדים."",
...
def chat():
    try:
        ...
        if user_audio:
            ...
            return f"id_list_message=f-/tmp/{phone}&go_to_folder=/1"
